...
import React, { useState } from 'react';
import { BanknoteIcon, Plus, Trash2, Crown, Check, Tag } from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {packageTemplates.map((template, index) => (
              <div key={index} className="border border-gray-200 rounded-xl p-4 hover:border-red-300 hover:shadow-md transition-all cursor-pointer"
                ...
                {template.badge && (
                  <span className={`inline-block px-2 py-0.5 text-xs font-medium rounded-full border ${getBadgeColor(template.badge)} mb-2`}>
                    {template.badge}
                  </span>
                )}
                <ul className="space-y-1 mb-4">
            ...
            <div key={pkg.id} className="border border-gray-200 rounded-xl p-6 bg-white shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  {pkg.badge && (
                    <span className={`px-3 py-1 text-xs font-semibold rounded-full border ${getBadgeColor(pkg.badge)}`}>
                      {pkg.badge}
                    </span>
                  )}
                  <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                ...
                <div>
                  ...
                  <button
                    type="button"
                    onClick={() => handlePopularPackageChange(pkg.id)}
                    className={`w-full px-4 py-2.5 border rounded-lg transition-all flex items-center justify-center gap-2 ${
                      pkg.isPopular 
                        ? 'bg-yellow-50 border-yellow-400 text-yellow-700' 
                        : 'bg-gray-50 border-gray-300 text-gray-500 hover:bg-gray-100'
                    }`}
                  >
                    <Crown className={`w-4 h-4 ${pkg.isPopular ? 'text-yellow-500' : 'text-gray-400'}`} />
                    {pkg.isPopular ? '⭐ Popular' : 'Mark as Popular'}
                  </button>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Badge <Tag className="w-4 h-4 inline ml-1 text-gray-400" />
                  </label>
                  <select
                    value={pkg.badge || ''}
                    onChange={(e) => updateItem(pricingPackages, setPricingPackages, pkg.id, 'badge', e.target.value)}
                  ...
                  <p className="text-xs text-gray-400 mt-1">Display badge on package card</p>
                </div>
              </div>
